...
const ERROR_LABELS = {
  silly_mistake: { label: 'Silly mistake', className: 'bg-amber-100 text-amber-700' },
  conceptual_gap: { label: 'Conceptual gap', className: 'bg-red-100 text-red-700' },
  time_pressure: { label: 'Time pressure', className: 'bg-purple-100 text-purple-700' },
}
...
function MistakeCard({ row, onSaved }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4">
      ...
      <p className="text-sm text-red-600 mb-1">Your answer: {yourAnswerText(row)}</p>
      <p className="text-sm text-green-700 mb-2">Correct answer: {correctAnswerText(q)}</p>
      <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 mb-3">{q.explanation}</p>
...
export function MistakeNotebookPage() {
  ...
  if (!user) return <p className="text-gray-500">Log in to see your mistake notebook.</p>
  if (loading) return <p className="text-gray-500">Loading...</p>
  if (error) return <p className="text-red-500">Couldn't load your mistakes. Please try again.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-semibold text-gray-900 mb-1">Mistake Notebook</h1>
      <p className="text-gray-500 text-sm mb-2">
        ...
        <Link to="/playbook" className="text-indigo-600 hover:underline">99th Percentile Playbook</Link>.
      </p>
