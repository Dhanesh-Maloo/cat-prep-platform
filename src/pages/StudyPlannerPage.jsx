...
export function StudyPlannerPage() {
  ...
  if (!user) return <p className="text-gray-500">Log in to build a study plan.</p>
  if (loading || syllabusLoading) return <p className="text-gray-500">Loading...</p>
  if (error) return <p className="text-red-500">Couldn't load your study plan. Please try again.</p>

  if (!targetExamDate) {
    return (
      <div className="max-w-md mx-auto text-center py-12">
        <h1 className="text-2xl font-semibold text-gray-900 mb-3">Study Planner</h1>
        <p className="text-gray-600 mb-4">
          Set your target CAT exam date in Settings first, and we'll build a week-by-week plan around it.
        </p>
        <Link to="/settings" className="text-indigo-600 hover:underline">Go to Settings →</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-gray-500 text-sm mb-6">Target exam date: {targetExamDate}</p>
          ...
          {plan.plan.map((week) => (
            <div key={week.week} className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <h2 className="font-semibold text-gray-800">Week {week.week}</h2>
                <span className="text-xs text-gray-400">{week.startDate} → {week.endDate}</span>
              </div>
              {week.focus === 'revision' ? (
                ...
                <ul className="space-y-1">
                  {week.subtopicIds.map((id) => (
                    <li key={id} className="text-sm">
                      <Link to={`/subtopic/${id}`} className="text-indigo-600 hover:underline">
