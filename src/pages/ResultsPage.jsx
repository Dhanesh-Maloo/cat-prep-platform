  ...
  if (!test || test.id !== mockTestId || !result) {
    return (
      <div className="text-center py-16">
        <p className="text-gray-500 mb-4">No completed attempt found for this test.</p>
        <Link to="/mock-tests" className="text-indigo-600 hover:underline">← Back to Mock Tests</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Correct" value={result.correct} accent="text-green-600" />
        <StatCard label="Incorrect" value={result.incorrect} accent="text-red-600" />
      </div>
      ...
      <h2 className="text-lg font-semibold text-gray-800 mb-3">Section-wise Breakdown</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        {Object.entries(result.sectionScores).map(([key, s]) => (
          <div key={key} className="bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="font-medium text-gray-800 mb-2">{key}</h3>
            <p className="text-2xl font-semibold text-gray-900">{s.raw}<span className="text-sm text-gray-400"> / {s.maxScore}</span></p>
            ...
            {wrongQuestions.map((q) => {
              ...
              return (
                <div key={q.id} className="bg-white border border-gray-200 rounded-lg p-4">
                  <p className="text-sm text-gray-500 mb-1">{q.section}</p>
      ...
      <div className="flex gap-4">
        ...
        <Link to="/mock-tests" className="text-gray-600 px-5 py-2.5 rounded-lg font-medium hover:bg-gray-100">
          Back to Mock Tests
        </Link>
      </div>
    </div>
